...
export function* decodeReplayGameEvents(contents, protocol){
    ...
    for (const event of decodeEventStream(decoder, protocol.game_eventid_typeid, protocol.game_event_types, true, protocol)){
        yield event;
    }
}       
...
export function* decodeReplayMessageEvents(contents, protocol){
    const decoder = new BitPackedDecoder(contents, protocol.typeinfos);
    for (const event of decodeEventStream(decoder, protocol.message_eventid_typeid, protocol.message_event_types, true, protocol)){
        yield event;
    }
}
...
export function* decodeReplayTrackerEvents(contents, protocol){
    ...
    for(const event of decodeEventStream(decoder, protocol.tracker_eventid_typeid, protocol.tracker_event_types, false, protocol)){
        yield event;
    }
}
...
export function unitTag(unitTagIndex, unitTagRecycle){ return (unitTagIndex << 18) + unitTagRecycle}
export function unitTagIndex(unitTag){ return (unitTag >> 18) & 0x00003fff}
export function unitTagRecycle(unitTag){return unitTag & 0x0003ffff}
